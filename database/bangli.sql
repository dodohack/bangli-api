...
CREATE TABLE IF NOT EXISTS `bangli`.`topics` (
  ...
  `author_id` INT NULL,
  `editor_id` INT NOT NULL COMMENT '编辑ID',
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bangli`.`topic_has_topic_view` (`id` INT, `channel_id` INT, `topic1_id` INT, `topic2_id` INT, `t2_title` INT, `t2_anchor_text` INT, `t2_guid` INT, `t2_aff_url` INT, `t2_type_id` INT, `t2_ranking` INT, `t2_type_slug` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bangli`.`attr_locations_view` (`id` INT, `loc_type` INT, `parent_id` INT, `text` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bangli`.`fe_attr_locations_view` (`id` INT, `loc_type` INT, `parent_id` INT, `name` INT, `slug` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bangli`.`fe_related_topic_view` (`id` INT, `topic_id` INT, `cat_id` INT, `type_id` INT, `title` INT, `anchor_text` INT, `guid` INT, `ranking` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `bangli`.`fe_deal_topics_view`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bangli`.`fe_deal_topics_view` (`id` INT, `author_id` INT, `editor_id` INT, `channel_id` INT, `type_id` INT, `location_id` INT, `lock` INT, `logo` INT, `ranking` INT, `status` INT, `guid` INT, `display_url` INT, `tracking_url` INT, `aff_platform` INT, `aff_id` INT, `title` INT, `title_cn` INT, `description` INT, `content` INT, `published_at` INT, `created_at` INT, `updated_at` INT, `ch_slug` INT, `tt_slug` INT, `view` INT, `word_count` INT, `share` INT, `comment` INT, `upvote` INT, `downvote` INT);
...
SELECT tht.id, t1.channel_id, tht.topic1_id, tht.topic2_id, t2.title as t2_title, t2.anchor_text as t2_anchor_text,
t2.guid as t2_guid, t2.aff_url as t2_aff_url, t2.type_id AS t2_type_id, t2.ranking as t2_ranking, 
...
SELECT `id`, `loc_type`, `parent_id`, `name` as `text` FROM `locations`;
...
SELECT `id`, `loc_type`, `parent_id`, `name`, `slug` FROM `locations`;
...
SELECT thp.id, thp.topic_id, thp.post_id, p.title, p.fake_published_at as published_at, s.`view`
...
SELECT thc.id, thc.topic_id, thc.cat_id, t.type_id, t.title, t.anchor_text, t.guid, t.ranking
...
-- -----------------------------------------------------
-- View `bangli`.`fe_deal_topics_view`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `bangli`.`fe_deal_topics_view` ;
DROP TABLE IF EXISTS `bangli`.`fe_deal_topics_view`;
USE `bangli`;
CREATE  OR REPLACE algorithm = merge VIEW `fe_deal_topics_view` AS
SELECT t.*, ch.`slug` AS ch_slug,  tt.`slug` AS tt_slug, 
s.`view`, s.`word_count`, s.`share`, s.`comment`, s.`upvote`, s.`downvote`
FROM topics AS t
LEFT JOIN channels AS ch ON t.`channel_id` = ch.`id`
LEFT JOIN topic_types AS tt ON t.`type_id` = tt.`id`
LEFT JOIN statistics AS s ON s.content_id = t.id AND s.content_type = 'topic'
WHERE t.`status` = 'publish' AND t.has_deal= 1;
