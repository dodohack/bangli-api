...
CREATE TABLE IF NOT EXISTS `bangli`.`posts` (
  ...
  `status` VARCHAR(15) NOT NULL DEFAULT 'draft' COMMENT '文章状态: 发布,草稿,待审,回收站, publish, draft, pending, trash',
...
CREATE TABLE IF NOT EXISTS `bangli`.`topics` (
  ...
  `type_id` INT NULL COMMENT 'Topic category',
  ...
  `featured` TINYINT(1) NULL COMMENT '1 - top level featured or 2 - category level featured, or 0 - not featured',
  `status` VARCHAR(15) NOT NULL DEFAULT 'draft' COMMENT '状态: 发布,草稿,待审,回收站, publish, draft, pending, trash',
  ...
  `image_idx` INT(2) NULL COMMENT 'Default image index, if it is not set, show images randomly',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `bangli`.`site_settings` (
  ...
  PRIMARY KEY (`id`))
ENGINE = InnoDB
COMMENT = '网站设置数据库，一些经常用到的全局设置会放到php文件，性能会更好';
...
CREATE TABLE IF NOT EXISTS `bangli`.`pages` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `editor_id` INT NULL COMMENT '编辑',
  `lock` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Same as lock in cms_posts',
  `status` VARCHAR(15) NOT NULL DEFAULT 'draft' COMMENT '页面状态: 发布,草稿,待审,回收站, publish, draft, pending, trash',
...
CREATE TABLE IF NOT EXISTS `bangli`.`email_subscriptions` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `email` VARCHAR(255) NOT NULL,
  `type` VARCHAR(45) NULL COMMENT 'Currently not in use. 订阅类型: 消息回复提醒，问题回复提醒，订阅帖子动态，订阅最新文章(必须支持文章分类订阅)  优惠提醒，每周精选，每月精选，活动促销',
  `frequency` INT NULL COMMENT 'Currently not in use. How often user like to receive an email: ',
  `sent_id` INT NULL COMMENT 'The lastest newsletter ID sent to the subscriber',
...
CREATE TABLE IF NOT EXISTS `bangli`.`advertises` (
  ...
  `position` VARCHAR(255) NOT NULL COMMENT 'The position of the ads, which has predefined value in the api server config ads.php',
  `rank` INT NULL COMMENT 'advertise ordering in the same position. can be from 1 to 100.',
  `location` ENUM('LOCAL', 'CN', 'GLOBAL') NOT NULL DEFAULT 'GLOBAL' COMMENT 'global, China or current country',
  `title` VARCHAR(255) NOT NULL,
  `description` TEXT(4096) NULL,
  `image_url` VARCHAR(255) NULL COMMENT 'Relative or absolute url address',
  `target_url` VARCHAR(255) NULL,
  `starts` DATETIME NULL COMMENT '广告开始展示的日期	',
  `ends` DATETIME NULL COMMENT '广告结束展示的日期',
  `published_at` DATETIME NULL,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_ads_channel_id`
    FOREIGN KEY (`channel_id`)
    REFERENCES `bangli`.`channels` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
CREATE INDEX `fk_ads_channel_id_idx` ON `bangli`.`advertises` (`channel_id` ASC);
...
CREATE TABLE IF NOT EXISTS `bangli`.`offers` (
  ...
  `starts` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
...
-- -----------------------------------------------------
-- Table `bangli`.`offer_filters`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `bangli`.`offer_filters` ;

CREATE TABLE IF NOT EXISTS `bangli`.`offer_filters` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL COMMENT 'User friendly filter name',
  `type` VARCHAR(40) NOT NULL COMMENT 'Filter type: content, extend, merchant_id',
  `content` TEXT NULL COMMENT 'Filter content, each row is a filter.',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '发布之日',
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后修改日期',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
COMMENT = 'Filter unwanted  offers by using MySQL builtin functions(regexp etc).';

CREATE UNIQUE INDEX `type_UNIQUE` ON `bangli`.`offer_filters` (`type` ASC);

CREATE UNIQUE INDEX `name_UNIQUE` ON `bangli`.`offer_filters` (`name` ASC);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bangli`.`fe_view_topics` (`id` INT, `author_id` INT, `editor_id` INT, `channel_id` INT, `type_id` INT, `location_id` INT, `lock` INT, `logo` INT, `ranking` INT, `featured` INT, `status` INT, `guid` INT, `display_url` INT, `tracking_url` INT, `aff_platform` INT, `aff_id` INT, `title` INT, `title_cn` INT, `description` INT, `content` INT, `published_at` INT, `created_at` INT, `updated_at` INT, `image_idx` INT, `ch_slug` INT, `tt_slug` INT, `view` INT, `word_count` INT, `share` INT, `comment` INT, `upvote` INT, `downvote` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `bangli`.`fe_view_ads`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bangli`.`fe_view_ads` (`id` INT, `channel_id` INT, `status` INT, `position` INT, `rank` INT, `location` INT, `title` INT, `description` INT, `image_url` INT, `target_url` INT, `starts` INT, `ends` INT, `published_at` INT, `created_at` INT, `updated_at` INT);
...
SELECT o.*
FROM offers AS o
WHERE o.`status` = 'publish' and o.`starts` <= now() and o.`ends` >= now();
...
-- -----------------------------------------------------
-- View `bangli`.`fe_view_ads`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `bangli`.`fe_view_ads` ;
DROP TABLE IF EXISTS `bangli`.`fe_view_ads`;
USE `bangli`;
CREATE  OR REPLACE algorithm = merge VIEW `fe_view_ads` AS
SELECT a.*
FROM advertises AS a
WHERE a.`status` = 'publish' and a.`starts` <= now() and a.`ends` >= now();
...
-- -----------------------------------------------------
-- Data for table `bangli`.`offer_filters`
-- -----------------------------------------------------
START TRANSACTION;
USE `bangli`;
INSERT INTO `bangli`.`offer_filters` (`id`, `name`, `type`, `content`, `created_at`, `updated_at`) VALUES (1, '内容过滤', 'CONTENT', '', DEFAULT, DEFAULT);
INSERT INTO `bangli`.`offer_filters` (`id`, `name`, `type`, `content`, `created_at`, `updated_at`) VALUES (2, '内容自动补齐', 'EXTEND', '', DEFAULT, DEFAULT);
INSERT INTO `bangli`.`offer_filters` (`id`, `name`, `type`, `content`, `created_at`, `updated_at`) VALUES (3, '商家过滤', 'MID', '', DEFAULT, DEFAULT);
